...
from .exception import FieldParseError, TableParseError, WhereParseError, UpdateParseError, JoinParseError, \
    LimitParseError, InsertParseError
            ...
            if isinstance(key, Term):
                ...
                if not isinstance(val, (tuple, list, dict)):
                    raise WhereParseError(
                        'Expect dict or item list/tuple for conditions to be connected by AND: "%s"' % val)
